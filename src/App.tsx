...
export default function App() {
  ...
  const handleInteraction = useCallback(() => {
    if (gameState === 'idle' || gameState === 'result' || gameState === 'tooEarly') {
      ...
      // 更新歷史紀錄
      setHistory(prev => [{ time: timeElapsed, date: Date.now() }, ...prev].slice(0, 5));
    }
  ...
  return (
    <div className={`min-h-screen transition-colors duration-200 select-none ${getBgColor()} flex flex-col items-center justify-center font-sans overflow-hidden`}>
      ...
      <AnimatePresence>
        {(gameState === 'idle' || gameState === 'result') && (
          <motion.div 
            ...
            id="stats-footer"
          >
            <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-zinc-200 shadow-sm" id="best-score-card">
            ...
            <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-zinc-200 shadow-sm" id="history-card">
              <div className="flex items-center gap-2 text-zinc-400 mb-1">
      ...
      {/* 快捷鍵提示 */}
      <div className="absolute top-6 left-6 hidden md:block" id="shortcut-hint">
